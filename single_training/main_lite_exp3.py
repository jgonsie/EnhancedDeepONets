...
if __name__ == "__main__":
    ...
    x_train, y_train, x_val, y_val = prepare_raw_data_items(training_data, train_split=200)
    ...
    net = DeepONet(layers_branch=[120,120,120,120,100], layers_trunk=[100,100,100,100], 
                   experiment='exp3', dimension='2D', seed=420, dtypeid=dtype)  

    loss_weights = generate_loss_weights(y_train)
    
    # Select model
    loss = 'H1+der'
    model = NeuralOperatorModel(net, grid, loss, LS=True, kappas = loss_weights,
                                regularizer = 10)


    sim_name = f'{loss}_test_9'
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(
        filepath=f'../results/exp3/{sim_name}_best.weights.h5',
        monitor='val_loss',
        save_best_only=True,
        mode='min',
    ...
    history = model.fit(x_train, y_train, validation_data=(x_val_r, y_val_r),
                        epochs=10000, batch_size=200, validation_batch_size=36,
                        callbacks=[nanchecker])
    
    model.save_weights(f'../results/exp3/{sim_name}_final.weights.h5')
