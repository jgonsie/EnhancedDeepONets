...
from SRC.utils import prepare_raw_data_items, generate_loss_weights
...
if __name__ == "__main__":
    ...
    x_train, y_train, x_val, y_val = prepare_raw_data_items(training_data, train_split=npoints)

    # Create learning model
    net = DeepONet(layers_branch=[120,120,120,120,100], layers_trunk=[100,100,100,100], 
                   experiment='exp3', dimension='2D', seed=seed, dtypeid=dtype)
                    
    loss_weights = generate_loss_weights(y_train)
    
    # Select model
    model = NeuralOperatorModel(net, grid, loss_function, kappas = loss_weights,
                                LS=True, regularizer = regularizer)

    sim_name = f'{loss_function}_LS-True_reg-{regularizer}_seed-{seed}_{npoints}'
    sim_dir = '../results/exp3/2.regularizer_init/'+sim_name
    ...
    x_val_r = {k:v[:200] for k,v in x_val.items()}
    y_val_r = {k:v[:200] for k,v in y_val.items()}
    
    nbatchval = int((200-npoints) / 5)
    history = model.fit(x_train, y_train, validation_data=(x_val_r, y_val_r),
                        epochs=15000, batch_size=npoints, validation_batch_size=nbatchval,
