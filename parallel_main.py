...
parser.add_argument("--LS", type=bool)
...
if __name__ == "__main__":
    ...
    net = DeepONet(layers_branch=[50,70], layers_trunk=[50,50,50,70], 
                   dimension='2D', seed=seed, dtypeid=dtype)
                    
    loss_weights = generate_loss_weights(y_train)
    
    # Select model
    model = NeuralOperatorModel(net, grid, loss_function, kappas = loss_weights,
                                LS=LS, regularizer = 10**(-2))

    sim_name = f'{loss_function}_LS-{LS}_{seed}'
    sim_dir = 'results/exp1/'+sim_name+'/'
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(
        filepath=sim_dir+'GD.weights.h5',
        monitor='val_loss',
        save_best_only=True,
        mode='min',
    ...
    history = model.fit(x_train, y_train, validation_data=(x_val, y_val),
                        epochs=10, batch_size=10, validation_batch_size=38,
                        callbacks=[checkpoint, nanchecker])

    df_history = pd.DataFrame(history.history)
    df_history.to_csv(sim_dir+'history.csv', index=False)
    
    plot1 = pp.plot_loss(df_history)
    plot2 = pp.plot_partialLosses_train(df_history)
    plot3 = pp.plot_partialLosses_val(df_history)
    plot1.savefig(sim_dir+'1LossVal.png', dpi=200, bbox_inches='tight')
    plot2.savefig(sim_dir+'2LossPartial.png', dpi=200, bbox_inches='tight')
    plot3.savefig(sim_dir+'3ValPartial.png', dpi=200, bbox_inches='tight')