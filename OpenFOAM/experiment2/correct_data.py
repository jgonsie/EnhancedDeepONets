...
if __name__ == "__main__":
    ...
    np.savetxt('exp2_corr_dirs.dat', failed_dirs, fmt='%s')
    
    # Perturbate failed simulation
    failed_sims_corr = failed_sims + 1e-9
    np.savetxt('exp2_corr_randomDT.dat', failed_sims_corr, fmt='%.9f')
    
    # Correct original file
    original_DT = np.loadtxt('exp2_randomDT.dat')
    for i in range(len(failed_sims)):  
        index = np.where(original_DT == failed_sims[i])[0]
        original_DT[index] = failed_sims_corr[i]
    np.savetxt('exp2_randomDT.dat', original_DT, fmt='%.9f')