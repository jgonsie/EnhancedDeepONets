...
int main(int argc, char *argv[])
{
    ...
    label tapeSizeMB = mesh.solutionDict().subDict("SIMPLE").lookupOrDefault<label>("tapeSizeMB",1096);
    Info << "Creating Tape, size: " << tapeSizeMB << endl;
    AD::createGlobalTape(tapeSizeMB);

	label nCells = T.size();

    std::vector<double> dTdDT(nCells, 0.0);

    // register inputs w.r.t. we want to diff.
	AD::registerInputVariable(DT.value());
	...
	for(int i=0; i<nCells; i++){
		...
		AD::interpretTape();
		dTdDT[i] = AD::derivative(DT.value());
